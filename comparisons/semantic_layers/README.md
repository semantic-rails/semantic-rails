...
questions over one shared Jaffle dataset? It is a capability comparison. It records no latency,
token or cost data, so it says nothing about performance. It runs without touching the active
...
- **Output check: 14 of 16 questions return matching normalized outputs across all six layers.**
  q07 and q16 do not, because the layers did not answer them from the same data:
  - Semantic Rails reads the full `jaffle_order_lifecycle` table (59,652 orders, 12 delivered
    months).
  - Cube, Malloy, Snowflake Semantic Views and KtX read the `comparison_order_lifecycle` view,
    which keeps only the 11 hand-authored lifecycle events (1 delivered month).
  - MetricFlow's model also reads the full table, but its committed answers were captured before
    the seed derived lifecycle rows for every order, so they match the restricted view instead.
...
- **The support labels are provisional.** The Semantic Rails authors wrote every layer's models
  ...
  although its q11 and q12 answers read precomputed customer columns (`lifetime_order_count`,
  `lifetime_spend_cents`), the same columns that MetricFlow's `precomputed` helper views read.
  Several layers are not yet modeled with native features they ship: MetricFlow conversion
  metrics and metric filters, Cube multi-stage measures and subquery dimensions, Malloy
  arbitrary-condition joins, and Snowflake range joins. An independent answer key, an executable
...
| Layer | Version | Captured | Re-runnable from this repo |
| --- | --- | --- | --- |
| Semantic Rails | 0.2.1 | 2026-09-23 (UTC) | yes |
| MetricFlow | `dbt-metricflow 0.11.0`, `dbt-duckdb 1.10.1` | by 2026-06-23 (exact date not recorded) | yes; installs pinned packages |
| Cube | `1.6.32` | by 2026-06-23 (exact date not recorded) | no; captured evidence only until its dependency advisories are resolved |
| Malloy | `@malloydata/cli 0.0.52` | by 2026-06-23 (exact date not recorded) | yes; installs the pinned CLI |
| Snowflake Semantic Views | Snowflake CLI + semantic view trial account | 2026-04-06 | needs a live Snowflake account |
| KtX | `ktx-sl 0.13.1` / KtX `a155c0b` | by 2026-06-23 (exact date not recorded) | yes; clones KtX at `a155c0b` |

The Semantic Rails runner records its engine version and run time in
`shared/results/semantic_rails/summary.json`.
...
| --- | --- | --- |
| Semantic Rails | 9 native | Semantic-model primitives; q11 and q12 read precomputed customer columns |
| MetricFlow | 2 native (q08, q16), 7 precomputed | Helper dbt views; native conversion metrics and metric filters not modeled yet |
| Cube | 3 workaround, 6 precomputed | Helper cubes and joined rollup filters; multi-stage measures and subquery dimensions not modeled yet |
| Malloy | 9 workaround | SQL sources and query-level filters; arbitrary-condition joins not modeled yet |
