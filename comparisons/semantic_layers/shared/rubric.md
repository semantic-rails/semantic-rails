...
| Malloy | Any `jaffle.sql(...)` block, as a source or a join declared inline, whose SQL appears in the executed SQL and isn't a passthrough. Malloy compiles that SQL into the query verbatim wherever the query reads it, whether directly, through a join or alias, or under an `extend`, so a declared join the query doesn't read doesn't count. |
