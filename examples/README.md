...
Worked Query IR payloads for the bundled synthetic `jaffle_shop` package. Each file is a
request body for `validate`, `compile` and `execute`, the same shape an agent sends through
MCP or `/api/v1/*`. `tests/semantic_rails/test_examples.py` checks every file against
`schemas/query_ir.v1.json` and the runtime, so they stay current.
...
| `qualified_metric_rollup.json` | Orders by store, counting only customers with four or more orders (a `metric_predicate` filter) |
