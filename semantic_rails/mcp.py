...
# Default row cap for MCP execute. Hosts warn about tool results over 10K tokens; 200 rows keeps a typical
...
MCP_SERVER_INSTRUCTIONS = (
    ...
    "by label.\n"
    "\n"
    "1. discover(terms) finds ids for the question's terms; inspect(object_id) and "
    "valid-values(dimension_id) show one object's details or a dimension's values.\n"
    "2. plan(intent) drafts Query IR; don't write Query IR from scratch. Run best.query_ir "
    'only when status is "ok" and there are no warnings. Otherwise why and warnings name what '
    "the draft misses: fix the Query IR or ask the user. out_of_scope or unrealizable means "
    "the package can't answer.\n"
    "3. execute(query) runs it and returns at most max_rows rows (truncated says more remain).\n"
    "\n"
    "Compose variants at query time, with no model change (shapes: see execute): any allowed "
    "aggregation of a measure, a filtered aggregate, a rolling window, a prior-period offset, "
    "period-to-date, cumulative, ratio, a conversion window, scoped_aggregate, aggregate_if, "
    "and metric_filters (metric_predicate) for conditions on an entity's metric; group_by "
    'dimension ids; where filters (op "in" for several values); time {temporal_role, grain, '
    "start, end}. time.end is exclusive. A window without time.grain returns one row per raw timestamp; a grain "
...
MCP_RESULT_SCHEMA: dict[str, Any] = {
    ...
    "description": (
        ...
        "the status, issue, request, and package fields are stable v2."
    ),
    "required": [
        "ok",
        "status",
    ...
    "properties": {
        ...
        "api_version": {"type": "string", "const": _INTERFACE},
...
MCP_RESULT_SCHEMA_SLIM: dict[str, Any] = {
    "type": "object",
    "description": "Semantic Rails MCP v2 result envelope.",
    "required": ["ok", "status"],
    "properties": {
        "ok": {"type": "boolean"},
        "status": {"type": "string"},
...
# The adapter also accepts "compact" as an alias for "full".
...
_VERBOSITY_MINIMAL: dict[str, Any] = {
    "type": "string",
    "enum": ["minimal", "compact", "full"],
    "default": "minimal",
}

# Every tool defaults to its smallest response.
TOOL_DEFINITIONS: tuple[ToolDefinition, ...] = (
    ...
    ToolDefinition(
        ...
        description=(
            ...
            "where[]={field,op,value}, metric_filters[]={expression:{kind:metric_predicate,"
            "entity,input,op,value},op:'=',value:true}, order_by[]={field,direction}. "
            "select.expression composes variants at query time: {metric} | "
            "{aggregation, measure} (any allowed aggregation) | {kind:aggregate, measure, "
            "aggregation, filter:{all:[{field,op,value}]}} | {kind:rolling, input, "
            "window:{unit,value}} | {kind:prior_period, input, offset:{unit,value}} | "
            "{kind:period_to_date, input, period} | {kind:cumulative, input} | "
            "{kind:conversion, base, converted, entity, window, matching_mode} | "
            "{kind:ratio|scoped_aggregate|aggregate_if|between|...}; the windowed kinds take no "
            "time.start. E.g. revenue from large orders: {kind:'aggregate', "
            "measure:'measure.jaffle.revenue_usd', aggregation:'sum', filter:{all:[{field:"
            "'dimension.jaffle_order_is_large_order', op:'=', value:true}]}}."
        ),
        input_schema=_schema(
            {
                "query": QUERY_SCHEMA,
...
PROMPT_DEFINITIONS: tuple[PromptDefinition, ...] = (
    ...
    PromptDefinition(
        ...
        description=(
            "Review existing Query IR with execute mode 'validate', then mode 'sql', before "
            "running it."
        ),
        arguments=(
    ...
    PromptDefinition(
        ...
        description="Validate, explain and preview a package-authored segment with the segment tool.",
        arguments=(
            {
                "name": "segment_id",
...
def list_resource_definitions() -> list[dict[str, Any]]:
    return [definition.to_dict() for definition in RESOURCE_DEFINITIONS]


def list_prompt_definitions() -> list[dict[str, Any]]:
    return [definition.to_dict() for definition in PROMPT_DEFINITIONS]
...
def _plan_detail(value: Any) -> str:
    """MCP plan's detail level; anything unknown gets the compact default, "query"."""
...
class SemanticLayerMCPAdapter:
                    ...
                    existing.append(
                        {
                            ...
                            "message": (
                                ...
                                "time window, or widen the time range. Use execute "
                                "with mode='sql' and verbosity='full' to inspect the "
                                "resolved plan."
                            ),
                        }
                    )
    ...
    def _handle_segment_action(self, arguments: dict[str, Any]) -> dict[str, Any]:
        """``segment``: validate, explain or preview a segment, at minimal
        verbosity unless the caller asks for more."""
