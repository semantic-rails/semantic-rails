...
def create_project(
    ...
    *,
    ...
    if overwrite:
        updates.extend(_retired_scaffold_model(project, package_id, files))
...
def _retired_scaffold_model(
    project: Path, package_id: str, new_files: dict[str, bytes]
) -> list[ProjectFileUpdate]:
    """Retire only an unchanged first model generated by this scaffold."""
    ...
    try:
        model = yaml.safe_load(model_path.read_bytes())["model"]
        times = model["times"]
        ...
        old_spec = ProjectSpec(
            ...
            first_model=FirstModel(
                ...
                primary_key=measures[f"{entity}_count"]["entity_key"],
                time_column=next(iter(times)),
                amount_column=(measures.get("total_amount") or {}).get("expr", ""),
                dimension_column=next(iter(dimensions), ""),
            ),
        )
        generated = project_scaffold_files(old_spec)
    except (KeyError, StopIteration, TypeError, AttributeError, yaml.YAMLError, SemanticLayerError):
        generated = {}
    if graph_path.read_bytes() != generated.get(
        "graph.yml"
    ) or model_path.read_bytes() != generated.get(old_path):
        raise SemanticLayerError(
            "INVALID_CONFIG",
