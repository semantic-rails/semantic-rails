...
def test_declared_foreign_key_keeps_schema_and_special_composite_target(tmp_path: Path) -> None:
    db_path = tmp_path / "keys.duckdb"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("main_marts.fct_orders; DROP", "OBJECT_NOT_FOUND"),  # one quoted name, not SQL
        ('"main_marts"."fct_orders"', "OBJECT_NOT_FOUND"),  # quotes are part of the name
        ("main.orders.extra", "INVALID_QUERY"),
        ("main..orders", "INVALID_QUERY"),
        ("main.orders.", "INVALID_QUERY"),
        ("../orders", "INVALID_QUERY"),
    ],
)
...
def test_dotted_names_are_not_relations_the_runtime_can_read(tmp_path: Path) -> None:
    ...
    with duckdb.connect(str(db_path)) as conn:
        ...
        conn.execute('CREATE TABLE "sales.v1".customers (customer_id INTEGER PRIMARY KEY)')
        conn.execute("CREATE TABLE sales.shipments (shipment_id INTEGER, customer_id INTEGER)")

    with open_duckdb(db_path) as warehouse:
        listed = list_tables(warehouse)
        described = describe_table(warehouse, "sales.orders")
        shipments = suggest_model(warehouse, "sales.shipments")

    assert [row["relation"] for row in listed] == ["sales.orders", "sales.shipments"]
    assert {column["name"] for column in described["columns"]} == {"id", "note"}
    assert shipments["foreign_keys"] == []  # the dotted customers table is no candidate


@pytest.mark.parametrize(
    ("schema", "table"), [("main", "sales orders"), ("sales-data", 'sales"orders')]
)
def test_special_relation_names_round_trip_to_an_executable_draft(
    tmp_path: Path, schema: str, table: str
) -> None:
    ...
    source = f'"{schema}"."{table.replace(chr(34), 2 * chr(34))}"'
    with duckdb.connect(str(db_path)) as conn:
    ...
    relation = table if schema == "main" else f"{schema}.{table}"

    with open_duckdb(db_path) as warehouse:
        listed = _by(list_tables(warehouse), "relation")[relation]
        described = describe_table(warehouse, relation)
        profiled = profile_columns(warehouse, relation, ["amount"])
        suggested = suggest_model(warehouse, relation)
    mcp = _session(
        create_architect_mcp_server(workspace_root=tmp_path),
        [
            (tool, {"relation": relation, "duckdb_path": str(db_path)})
            for tool in ("describe_table", "profile_columns", "suggest_model")
        ],
    )

    assert (listed["schema"], listed["name"]) == (schema, table)
    assert described["primary_key"] == ["id"] and profiled["columns"][0]["samples"] == [110]
    assert [row["relation"] for row in (described, profiled, suggested, *mcp)] == [relation] * 6
    assert suggested["upsert_model"]["relation"] == relation
    assert mcp[2]["upsert_model"] == suggested["upsert_model"]
    mutation = ArchitectProject(project, workspace_root=tmp_path).upsert_model(
        **suggested["upsert_model"]
    )
...
def test_package_database_resolves_as_the_runtime_does() -> None:
    package = Path(repo_root()) / "configs" / "semantic_rails" / "jaffle_shop"

    assert package_duckdb_path(package) == str(Path(repo_root()) / "data" / "jaffle_shop.duckdb")
