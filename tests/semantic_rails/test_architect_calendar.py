...
def test_calendar_false_waits_for_the_date_dimensions_to_go(workspace: Path) -> None:
    project = _project(workspace)
    project.upsert_model(**_calendar())
    before = project_revision(workspace / "shop")

    with pytest.raises(SemanticLayerError, match="only a calendar may have kind: date dimensions"):
        project.upsert_model(**_calendar(calendar=False))
    assert project_revision(workspace / "shop") == before
