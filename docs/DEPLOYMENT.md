...
| `SEMANTIC_RAILS_ALLOW_DB_RESEED` | Set to `1` to let the runtime rebuild a DuckDB `package.default_db` from `package.seed` even though that seed did not build the file (or the file changed since). Off by default: without it, such a database is reported when it lacks relations the package reads, never replaced. A file the runtime cannot open (for example while another process writes it) is never replaced, even with this set. |
