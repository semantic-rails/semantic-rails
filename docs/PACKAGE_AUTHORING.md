...
confirmation to No. Type `cancel` or press Ctrl-C to stop without writing. In
arrow-key pickers a suggested default is a placeholder: typing replaces it.
Managing an existing object starts every prompt at its saved value, even a value
the menu does not otherwise list, so pressing Enter throughout leaves it as it
was. Changing a measure's default aggregation warns and names the metrics whose
numbers change. Successful edits are parse-validated and can be restored with
`undo` during the same REPL session. A failed parse restores the original files
automatically.
