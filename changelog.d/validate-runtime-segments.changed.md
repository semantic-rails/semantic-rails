- `validate runtime` (and `project validate --mode runtime`) also runs each segment's
  ...
  warehouse-free: for a membership value on a text dimension, which it can't check
  against the column, it adds a `SEGMENT_VALUES_UNCHECKED` warning that names those
  commands.
