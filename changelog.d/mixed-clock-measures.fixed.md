- A query whose clock a measure lacks no longer times that measure by the first of
  ...
  `temporal_role_overrides`. A measure with a single clock is still aligned by it.
  Package validation rejects a metric whose declared clock would be refused this way.
