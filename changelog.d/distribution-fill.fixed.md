- A `distribution` with `time.fill: true` returned wrong values: every entity entered every
  ...
  over a per-entity `rolling` or `prior_period` value, which counted entities in periods
  where they had no rows, and a filled `distribution` on a non-default calendar (for example
  fiscal quarters). See [docs/QUERY_IR_SCHEMA.md](docs/QUERY_IR_SCHEMA.md).
