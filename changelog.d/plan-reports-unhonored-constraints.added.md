- `plan` reports the parts of a question its draft doesn't honor. A dropped or different time
  window, a ranking that loses its limit, sort or ranked dimension, a named filter value the
  draft neither filters on nor groups by, or filters that require one field to equal two
