- A new `row_filter` package policy limits a relation's rows to a trusted request attribute,
  ...
  it can't enforce. A warehouse error on a parameterized statement is now raised without the
  driver's message. See [docs/PACKAGE_AUTHORING.md](docs/PACKAGE_AUTHORING.md).
