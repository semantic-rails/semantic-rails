...
| Filters on a `derived` metric or its inputs | A derived metric with a filtered input is skipped, since its input would be computed unfiltered. A filter on the metric itself is dropped with a warning. |
