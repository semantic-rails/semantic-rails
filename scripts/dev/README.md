...
| `verify_move.py` | Prove that a change only moves code between `semantic_rails` modules: every top-level statement keeps its exact text and AST, every name it reads (including lazy imports) resolves to the same definition, only lazy forwarders are new, no moved code reads `__name__`/`__file__`, comments are unchanged, no test patches a moved name at its old path, and every existing import still resolves. `python scripts/dev/verify_move.py <base-ref> [<head-ref>]`. |
