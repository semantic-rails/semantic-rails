...
| `capture_dialect_sql.py` | Compile (without running) the same queries once per supported dialect and write each statement's SQL and result-column mapping to a JSON file; it exits 1 if any statement fails to compile. `--compare GOLDEN AFTER` lists every statement that changed and fails on any compile failure. A pure code move must leave all of them byte-identical. |
| `verify_move.py` | Prove that a change only moves code between `semantic_rails` modules: every top-level statement keeps its exact text and AST (nested imports as placeholders), every name it reads from module scope (including lazy imports) resolves to the same definition, only lazy forwarders to functions are new, no moved code reads `__name__`/`__file__` or touches a rebound global, moved public and `__all__` names stay reachable from the old module, comments are unchanged, no test patches a moved name at its old path, and every existing import still resolves. It reads only this repository: importers and patches in other repositories need their own check. `python scripts/dev/verify_move.py <base-ref> [<head-ref>]`. |
